...
/////////////////// ADD SONG FUNCTIONALITY ////////////////////
	$("#addSong").on("click", () => {
		...
		let album = $("#album").val();
		$("#more").remove();
		$("#musicContent").append(`<div class="row"> ${song} | <strong>${artist}</strong> | ${album} <button class="delete btn btn-danger" type="button">Delete</button><div><div><button type="button" id="more" class="btn btn-default">More</button></div>`);
		$("#addMusicView").hide();
		$("#listMusicView").show();
...
/////////////////////// MORE BUTTON FUNCTIONALITY /////////////////////////
	...
	function repopulateContainer(data) {
		...
		for(let songNumberName2 in data.songs){
			...
			moreSongsContent += `<div class="row">${songName2} | <strong>${songArtist2}</strong> | ${songAlbum2} <button class="delete btn btn-danger" type="button">Delete</button></div>`;
		}
